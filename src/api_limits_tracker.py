...
class ApiLimitsTracker:
    def __init__(self, provider_name: str, model_name: str, limits: dict) -> None:
        self.prvdr_name = provider_name
        self.model_name = model_name
        self.limits = limits
        self.cooldown_until = 0  # Timestamp until which this model is in cooldown
    ...
    def can_handle(self, num_of_tokens: int) -> bool:
        ...
        rps_ok = (L["requests_per_second"] == -1) or (len(self.deque_req_sec) < L["requests_per_second"])
        rpm_ok = (L["requests_per_minute"] == -1) or (len(self.deque_req_min) < L["requests_per_minute"])
        rph_ok = (L["requests_per_hour"] == -1) or (len(self.deque_req_hr) < L["requests_per_hour"])
        rpd_ok = (L["requests_per_day"] == -1) or (len(self.deque_req_day) < L["requests_per_day"])

        tpm_ok = (L["tokens_per_minute"] == -1) or (self.total_tok_min + num_of_tokens <= L["tokens_per_minute"])
        tph_ok = (L["tokens_per_hour"] == -1) or (self.total_tok_hr + num_of_tokens <= L["tokens_per_hour"])
        tpd_ok = (L["tokens_per_day"] == -1) or (self.total_tok_day + num_of_tokens <= L["tokens_per_day"])

        all_ok = rps_ok and rpm_ok and rph_ok and rpd_ok and tpm_ok and tph_ok and tpd_ok
        return all_ok
    ...
    def get_wait_time(self, num_of_tokens: int) -> float:
        ...
        # Request limits
        if L["requests_per_second"] != -1 and len(self.deque_req_sec) >= L["requests_per_second"]:
            wait_times.append(max(0, 1.0 - (now - self.deque_req_sec[0])))

        if L["requests_per_minute"] != -1 and len(self.deque_req_min) >= L["requests_per_minute"]:
            wait_times.append(max(0, 60.0 - (now - self.deque_req_min[0])))

        if L["requests_per_hour"] != -1 and len(self.deque_req_hr) >= L["requests_per_hour"]:
            wait_times.append(max(0, 3600.0 - (now - self.deque_req_hr[0])))

        if L["requests_per_day"] != -1 and len(self.deque_req_day) >= L["requests_per_day"]:
            wait_times.append(max(0, 86400.0 - (now - self.deque_req_day[0])))

        # Token limits
        def calc_token_wait(deque_obj, limit_val, total_val, window_sec):
            if limit_val == -1: return 0.0
            if num_of_tokens > limit_val: return float('inf')
            
            # If we already exceed limit, find how long until enough tokens expire
            temp_total = total_val + num_of_tokens
            if temp_total <= limit_val: return 0.0
        ...
        wait_times.append(calc_token_wait(self.deque_tok_min, L["tokens_per_minute"], self.total_tok_min, 60))
        wait_times.append(calc_token_wait(self.deque_tok_hr, L["tokens_per_hour"], self.total_tok_hr, 3600))
        wait_times.append(calc_token_wait(self.deque_tok_day, L["tokens_per_day"], self.total_tok_day, 86400))
